...
const FormSwitcher = () => {
  ...
  return (
    <div className="flex items-center justify-center max-h-fit p-2">
      <div className="relative w-full max-w-[900px] h-auto max-h-[600px] bg-gradient-to-br from-pink-400 to-pink-600 rounded-xl shadow-lg overflow-y-auto overflow-x-hidden">
        {/* Sign Up Form */}
        <div
          className={`w-1/2 h-full flex flex-col justify-center items-center bg-background transition-opacity duration-500 ease-in-out {opacity-0 z-10' : opacity-100 z-10'}`}
        >
          <form
            onSubmit={handleSubmit}
            className="w-full max-w-[350px] p-8 space-y-6"
          >
        ...
        <div
          className={`absolute inset-0 flex transition-all duration-1000 ease-in-out ${
            isSubmitted ? "translate-x-1/2" : "translate-x-full"
          }`}
        >
          {/* Sign In Form */}
          <div
            className={`w-1/2 h-full flex items-center justify-center transition-opacity ease-in-out ${
